...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    if ($_SERVER['CONTENT_TYPE'] === 'application/json') {
        ...
        if ($Verified == TRUE) {
            ...
            //Update article
            if ($articles->update_article()) {
                echo json_encode(array('message' => 'Article ' . $articles->article_id . ' is updated successfully.'));
            } else {
                echo json_encode(array('message' => 'Article ' . $articles->article_id . ' can not be updated right now.'));
            }
        } else {
